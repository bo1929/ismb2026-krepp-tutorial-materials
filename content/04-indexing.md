...
## Installing WoL and other available indexes
...
This particular index is small enough to fit in your laptop's memory during the tutorial.

Run `cat data/index-WoLv1-tiny/metadata-*` to see the configuration and verify the download.
??? question "Expected output:"
    `data/index-WoLv1-tiny/`
...
```bash
krepp inspect -i data/toy-index | head -n 15
```
??? question "Expected output:"
    ```
    Backbone tree: NA
...
If you only want to test different parameters and options without overwriting the bundled index, use a different `-o` directory to create a new index (for example `tmp-index`).
...
??? more "Advanced: batching for ultra-large datasets and FracMinHash"
    ...
    ```bash
    krepp index -o /path/to/index -i /path/to/input.tsv -t /path/to/tree.nwk --num-threads 8 -m 100 -r 99 --no-frac
    krepp index -o /path/to/index -i /path/to/input.tsv -t /path/to/tree.nwk --num-threads 8 -m 100 -r 98 --no-frac
    krepp index -o /path/to/index -i /path/to/input.tsv -t /path/to/tree.nwk --num-threads 8 -m 100 -r 97 --no-frac
    ```
    or, alternatively
    ```bash
    krepp index -o /path/to/index -i /path/to/input.tsv -t /path/to/tree.nwk --num-threads 4 -m 100 -r 2 --frac
    ```
    This is faster but uses more memory during index construction, despite resulting in an index of the same size.
    If this doesn't work well for your task, you can later build the remaining 47% of the index to complete it to 50% by running
    ```bash
    krepp index -o /path/to/index -i /path/to/input.tsv -t /path/to/tree.nwk --num-threads 4 -m 100 -r 46 --frac
    ```
    or, if you have a low-memory machine, you can iterate over partitions
    ```bash
    seq 1 46 | xargs -I{} -P2 bash -c "krepp index -o /path/to/index -i /path/to/input.tsv -t /path/to/tree.nwk --num-threads 8 -m 100 -r {} --no-frac"
    ```
