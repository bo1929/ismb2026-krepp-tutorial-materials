#!/usr/bin/env python3
"""
Assign role fields from taxonomic LCA between queries (query_taxonomy.tsv) and
references (NCBI lineages). Updates data/query_info.tsv and data/reference_info.tsv.

Usage:
  python3 scripts/update_taxonomy_roles.py
  python3 scripts/update_taxonomy_roles.py --dry-run
"""
...
# NCBI ranks we emit as role (deepest shared rank name).
RANK_PRIORITY = {
    "species": 7,
    "subspecies": 7,
    "strain": 7,
...
def role_from_lca(lca_id: str | None, rank_cache: dict[str, str]) -> str:
    if not lca_id:
        return "kingdom"
...
def best_match(
    ...
    """Return (target_id, role, lca_taxid) for closest target."""
    best_tid = ""
    best_role = "kingdom"
    best_lca = ""
    best_depth = -1
    ...
    for tid, tpath in targets.items():
        ...
        role = role_from_lca(lca, rank_cache)
        prio = RANK_PRIORITY.get(role, 0)
        if depth > best_depth or (depth == best_depth and prio > best_prio):
            ...
            best_role = role
            best_lca = lca or ""
    return best_tid, best_role, best_lca
...
def write_reference_info(path: Path, header: list[str], rows: list[dict]) -> None:
    out_header = [
        "i", "label", "role", "profile_taxid", "profile_genus",
    ...
    with path.open("w", encoding="utf-8", newline="") as fh:
        ...
        for r in rows:
            w.writerow([
                r["i"], r["label"], r["role"], r["profile_taxid"], r["profile_genus"],
...
def write_query_info(path: Path, header: list[str], rows: list[dict]) -> None:
    out_header = ["taxid", "taxon", "role", "accession", "organism"]
    with path.open("w", encoding="utf-8", newline="") as fh:
...
def main() -> int:
    ...
    for row in qi_rows:
        ...
        if not qpath:
            continue
        _, role, _ = best_match(qtid, qpath, ref_targets, rank_cache)
        row["role"] = role
    ...
    for row in ref_rows:
        ...
        if not rpath:
            continue
        _, role, _ = best_match(rtid, rpath, query_targets, rank_cache)
        row["role"] = role

    sys.stderr.write("Query roles:\n")
    for row in sorted(qi_rows, key=lambda r: r.get("taxid", "")):
        sys.stderr.write(f"  {row.get('taxid')}\t{row.get('organism', '')[:40]}\t{row.get('role')}\n")

    sys.stderr.write("\nReference roles:\n")
    for row in ref_rows:
        sys.stderr.write(f"  {row['i']}\t{row['label'][:28]}\t{row['role']}\n")
